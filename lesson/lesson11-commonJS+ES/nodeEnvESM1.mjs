...
//export 是key word，不是对象; cmj中module是属性 用 require 使用
...
// CMJ: 运行时 同步、对象导出 值拷贝风格 像“函数包一层require / module.exports 的脚本”
// ESM:编译期 支持异步 基于绑定的模块系统 偏语言级模块（能tree-shaking 等优化）
...
// 运行时行为：this/顶层/严格模式
// CMJ:顶层 this === module.exports 顶层不是“自动严格模式”（可以手动 'use strict'）可以访问 __filename、__dirname、arguments 等 CJS 注入的变量
...
// 代码优化：
// CMJ: 由于 require 是普通函数调用 + 动态拼路径，编译器很难静态分析出到底用到了哪些成员 打包工具很难做真正的 tree-shaking（通常只能靠启发式）
// ESM:import 必须是静态的，路径是常量字符串，导入的标识符也是固定的;打包器能准确知道“用到了哪些导出”，从而做 tree-shaking，丢掉没用的代码;这也是现代前端库都推荐提供 ESM 入口的原因之一

// 动态性：
// CMJ:更动态  require(someVar) 可以用变量决定模块路径
