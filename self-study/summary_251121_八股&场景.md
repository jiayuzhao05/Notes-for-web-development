...
|          |                                                              | 在客户端js脚本使用import和export，设置`<script type="module">` |

| 浅拷贝                             | 深拷贝                                     | 实现方式                                                     |
| ---------------------------------- | ------------------------------------------ | ------------------------------------------------------------ |
| 复制对象引用，不创建新对象         | 创建新对象（包括嵌套对象）                 | 深：_.cloneDeep() loads                                      |
| 原始和新对象共享相同嵌套对象       | 原始对象和新对象独立，修改一个不影响另一个 | JSON.parse() JSON.stringify()不能复制对象、undefined、循环引用 |
| 修改嵌套对象，同时影响原始和新对象 |                                            | 手写循环引用                                                 |
| Object.assign()                    |                                            | 深：浏览器原生支持`structuredClone()` 使用结构化克隆算法     |



**PWA** progress web app 渐进式网页应用 类似于原生应用体验

| 离线可用   | Service Worker实现资源缓存，弱网、离线访问核心内容           |
| ---------- | ------------------------------------------------------------ |
| 快速加载   | 缓存和资源预加载 提升应用加载速度 对移动端有价值             |
| 响应式设计 | 应用适配设备尺寸，类似常规响应式开发                         |
| 类原生体验 | 通过manifest文件配置，实现添加到主屏幕、全屏显示灯特性，配合交互动效，提供接近原生应用体验 |
...
|              | 根据预设分片大小计算上传分片数量                             |
|              | 为每个分片生成唯一hash值为标识                               |
| 上传准备     | 将文件分片信息和hash值发送给服务端，携带一个唯一标识符标记文件 |
|              | 服务端记录分片信息                                           |
| 分片上传     | 上传文件分片，信息保存在本地                                 |
|              | 服务端保存已上传分片                                         |
