...
基本类型：string，symbol，number，boolean，undefined，null （stack）
引用类型：object Array、Date、RegExp （heap）
引用类型有自己内置的方法，也可以自定义其他方法用来操作数据，而基本数据类型不能像引用类型那样有自己的内置方法对数据进行更多的操作

console.log(null == undefined) // true
当我们试图访问某个不存在的或者没有赋值的变量时，就会得到一个 *undefined* 值。*Javascript* 会自动将声明是没有进行初始化的变量设为 *undifined*
...
JS包装类型：
包装对象 当基本类型以对象方式使用 JS转换成包装类型 new对象 内容和基本类型的一样 操作完成再去访问 临时对象销毁 再访问就是undefined

隐式转换：强制转换为基础  遇到下面情况 JS自动转换数据类型 用户不可见
1.不同类型的数据互相运算
123 + 'abc' // "123abc"
...
Boolean() 除了null，undefined，NaN，''，0，false，其他都是true

reload:加法运算符是在运行时决定，到底是执行相加，还是执行连接
...
按位运算
按位异或：两个位都为1，结果为0，否则为1（转为 32 位二进制整数）
按位移位：按位移位运算符`<<`和`>>`会将所有位向左或者向右移动指定的数量 = 将数字乘以/除以 2 的指定数的次方
`<<`：乘以 2 的指定数次方
`>>`：除以 2 的指定数次方

JS执行环境：全局 函数环境 eval（不推荐）
栈管理执行上下文 call stack：栈底永远是全局环境执行上下文 栈顶是当前执行函数执行上下文
如果执行上下文超出栈分配空间 堆栈溢出 常见于递归调用 没有终止条件 死循环 Uncaught RangeError: Maximum call stack size exceeded

所有 *window* 对象的属性拥有全局作用域 window.name、window.location、window.top

自由变量：要到创建这个函数的那个域”。作用域中取值，这里强调的是“创建”，而不是“调用”（静态作用域）
...
this指向
函数体中，非显式或隐式地简单调用函数时，在严格模式下，函数内的 *this* 会被绑定到 *undefined* 上，在非严格模式下则会被绑定到全局对象 *window/global* 
使用 *new* 方法调用构造函数时，构造函数内的 *this* 会被绑定到新创建的对象
通过 *call/apply/bind* 方法显式调用函数时，函数体内的 *this* 会被绑定到指定参数的对象
通过上下文对象调用函数时，函数体内的 *this* 会被绑定到该对象
