...
// 点击并观察浏览器的 Elements的变化
    ...
    <script>
        ...
        container.addEventListener('click',function(e){
            const target = e.target.closest('.btn') //eventobject 点击事件对象 e.target 触发事件最具体最深的元素 匹配祖先
            console.log(e,'-',e.target) //e是object
            //closest用法

            //target存不存在 在不在container中
            if(!container.contains(target) || !target) return
            console.log("hello")
        })
    </script>
</body>
